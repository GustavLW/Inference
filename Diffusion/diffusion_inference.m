...
for i = 1:N
    ...
    betaMSD = beta;
for c = 1:4
    dt = observed_cells{end}*2^(c-1);
    for k   = c:c:K-c
        x   = squeeze(ECEAT(:,:,k))';
        axn = squeeze(ECEAT(:,:,k+c))';
        for i = 1:N
            xn        = squeeze(ECEAT(i,:,k+c))';
            xi        = x(:,i);
...
for c = 1:4 
    ...
    title([num2str(10*2^(c-1)) ' minutes between observations'])
end
...
legend('Our method','MSD','Location','northwest')
title([num2str(N) ' cells. Minutes between observations: ' num2str(10*2^(c-1))])
drawnow;
pause(1)
end
