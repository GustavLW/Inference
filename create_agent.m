function agent = create_agent(type,T)
if type == 1
    theta = [15*rand-15 5*rand];
elseif type == 2
    k1 =    - 5 + 10*rand;
    k2 = k1 - 3 + 2*rand;
    a1 =    - 5 + 5*rand;
    a2 = a1 - 3 - 5*rand;
    l1 =    - 8 + 5*rand;
    l2 = l1 + 1 + 3*rand;
    theta = [k1 l1 a1 k2 a2 l2];
end

agent = struct('sigma',NaN,'U_param',theta.*ones(T,length(theta)),...
    'U_velo',(1 + 0.02*abs(randn(size(theta)))).*ones(T,length(theta))/10,...
    'fitness',-Inf*ones(T,1),'penalty',zeros(T,1),...
    'U_best',theta.*ones(T,length(theta)),'Bfitness',-Inf*ones(T,1),'Bpenalty',zeros(T,1),'dev',10^20*ones(T,1),'Bdev',10^20*ones(T,1));